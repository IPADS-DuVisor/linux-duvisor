...
#include "../../include/uapi/misc/dv_driver.h"
#include "csr.h"

#define IOCTL_DRIVER_NAME "/dev/dv_driver"
...
int pass_phys_mem(void) {
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_QUERY_PFN, &pfn) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_QUERY_PFN");
        return -1;
    }
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_RELEASE_PFN, &pfn) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_RELEASE_PFN");
        return -1;
    }
...
int pass_huret(void) {
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_QUERY_PFN, &test_buf_pfn) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_QUERY_PFN");
        return -1;
    }
    ...
    deleg_info[1] = 1 << 0;
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_REQUEST_DELEG, deleg_info) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_REQUEST_DELEG");
        return -1;
    }

    if (ioctl(fd_ioctl, IOCTL_DUVISOR_REGISTER_VCPU) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_REGISTER_VCPU");
        return -1;
    }
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_UNREGISTER_VCPU) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_UNREGISTER_VCPU");
        return -1;
    }
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_RELEASE_PFN, &test_buf_pfn) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_RELEASE_PFN");
        return -1;
    }
...
int pass_ioctls(void) {
    ...
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_GET_API_VERSION, &value) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_GET_API_VERSION");
        return -1;
    }
    printf("Value is %lx\n", value);
    ...
    sm_info[0] = 0xdead000;
    sm_info[1] = 0x1000;
    if (ioctl(fd_ioctl, IOCTL_DUVISOR_REGISTER_SHARED_MEM, sm_info) < 0) {
        perror("Error ioctl IOCTL_DUVISOR_REGISTER_SHARED_MEM");
        return -1;
    }
